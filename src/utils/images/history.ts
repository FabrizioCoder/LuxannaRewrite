...
async function makeLabel(
	...
	const icon = (
		await getIcon("14.2.1", participant.championId.toString(), 91)
	).roundCorners(16);
	...
	const ward = participant.item6
		? (await getItem("14.2.1", participant.item6)).roundCorners(6)
		: null;
...
export async function makeMatchHistory(
	...
	const icon = await getIcon("14.2.1", participant.championId.toString());
	...
	for (let i = 0; i < build.length; i++) {
		const item = build[i]!;
		const icon = await getItem("14.2.1", item);
